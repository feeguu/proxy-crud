...
export class PostgresAdminRepository extends Repository<Admin> {
  ...
  async findOne(id: string): Promise<Admin | null> {
    const res = await this.database.getClient().query(`SELECT * FROM admins WHERE id = ${id}`) as QueryResult<any>;
    if (res.rowCount === 0) {
      return null;
    }
  ...
  async save(entity: Admin) {
    const query = `INSERT INTO admins (email, password) VALUES ('${entity.email}', '${entity.password}')`;
  ...
  async findByColumn(column: string, value: string): Promise<Admin | null> {
    const res = await this.database.getClient().query(`SELECT * FROM admins WHERE ${column} = '${value}'`) as QueryResult<any>;
    if (res.rowCount === 0) {
      return null;
    }
    const admin = res.rows[0];
